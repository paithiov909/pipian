#' @noRd
#' @keywords internal
R6_CabochaTbl <- R6::R6Class("CabochaTbl",
  public = list(
    texts = NULL,
    rcpath = NULL,
    tbl = NULL,
...
#'
#' Calls cabocha -f3 command via \code{system()}, then returns results as a tibble.
...
#'   \item tbl2graph(): converts tbl to igraph graph object.
#'   \item plot(): simply plots the syntactic tree.
#' }
#'
#' @import R6
#' @import dplyr
...
CabochaTbl <- function(texts, rcpath = NULL, force.utf8 = FALSE) {
  ...
  return(R6_CabochaTbl$new(texts, rcpath, df))
}
